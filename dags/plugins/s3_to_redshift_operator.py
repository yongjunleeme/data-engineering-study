...
class S3ToRedshiftOperator(BaseOperator):
    ...
    def execute(self, context):
        ...
        credentials = self.s3.get_credentials('us-west-2')
        ...
        if self.previous_task_id is not None:
            max_created = context['task_instance'].xcom_pull(task_ids=self.previous_task_id, key='max_created') 
            field = context['task_instance'].xcom_pull(task_ids=self.previous_task_id, key='field')
            queries += "BEGIN;DELETE FROM {schema}.{table} WHERE {field} > '{value}';".format(
                schema = self.schema,
                table = self.table,
                field = field,
                value = max_created
        ...
        # Runs explicit grant operation regardless
        # since some tables can be re-created outside of this code
        # and it will cause "permission error"
        queries = """GRANT SELECT ON TABLE {schema}.{table} TO GROUP analytics_users;
        """.format(
            table=self.table,
            schema=self.schema
        )

        self.log.info('Executing permission grant command... ' + queries)
        self.hook.run(queries, self.autocommit)
